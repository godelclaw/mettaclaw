...
class PlaceholderSendGuardTests(unittest.TestCase):
    def test_placeholder_sends_refused_loudly(self):
        sys.path.insert(0, os.path.join(ROOT, "channels"))
        import telegram
        verdict = telegram.send_message("...")
        self.assertIn("refused", verdict)
        self.assertTrue(telegram._placeholder_only("…"))
        self.assertFalse(telegram._placeholder_only("pmSum_factor is CLOSED"))
