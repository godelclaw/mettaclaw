...
class Settings:
    ...
    def from_env(cls, root=None, store=None):
        ...
        return cls(
            ...
            bwrap=os.environ.get("METTACLAW_BWRAP", "bwrap"),
            timeout_seconds=max(
...
def check_semantics(candidate_path, settings):
    """Evaluate a candidate with no ambient home, network, or writable host tree."""
    system_path = "/usr/local/bin:/usr/bin:/bin"
    sandbox_path = system_path
    python_mounts = []
    python_environment = []
    if settings.python_env is not None:
        sandbox_path = "/opt/python/bin:" + system_path
        python_mounts = [
            "--dir",
            "/opt/python",
            "--ro-bind",
            str(settings.python_env),
            "/opt/python",
        ]
        python_environment = [
            "--setenv",
    ...
    command = [
        ...
        "--dir",
        "/opt",
    ] + python_mounts + python_environment + [
        "--ro-bind",
        str(settings.petta_root),
        "/opt/petta",
        "--ro-bind",
    ...
    try:
        ...
        raise SelfModError("semantic sandbox failed: %s" % type(exc).__name__)
    if result.returncode != 0 or "ERROR:" in (result.stderr or ""):
        raise SelfModError(
            "sandboxed semantic check rejected candidate: "
            + _sanitized_detail(result.stderr, settings)
        )
    return {"kind": "bwrap-petta-eval", "status": "pass"}
