...
class ActivityBatchTests(unittest.TestCase):
    ...
    def test_policy_selected_autonomy_renews_only_after_an_idle_wait(self):
        loop = (ROOT / "src" / "loop.metta").read_text(encoding="utf-8")
        ...
        self.assertIn("AUTONOMOUS_BURST", loop)
        self.assertIn("loop_modes.burst_budget", loop)
        self.assertNotIn("(quote claw23)", loop)
